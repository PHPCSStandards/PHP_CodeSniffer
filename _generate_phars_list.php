...
foreach ($filesGroupedByVersion as $version => $files) {
    ...
    foreach ($files as $file) {
        $fileSize = humanReadableFilesize("./phars/" . $file);
        $lastModifiedDate = shell_exec(escapeshellcmd("git log -1 --pretty=\"format:%cs\" -- phars/" . $file));
...
$template = file_get_contents('_phars.html.template');

$output = str_replace('<!-- {{ PHARS }} -->', $html, $template);

file_put_contents('phars.html', $output);

echo "phars.html generated successfully.\n";
